...
namespace tb_512
{
    ...
    bool
    Checker::is_done(void)
    {
        return this->done;
    }

    void
    Checker::set_max_size(uint32_t arg)
    {
        if (arg < 64U)
        {
            this->max_size = 64U;
    ...
    Checker::check(void)
    {
        ...
        while (true)
        {
            ...
            if (false)
            {
                ...
                os << SP << "len:" << SP << hex << setfill('0') << uppercase << setw(8) << sig_len;
            ...
            {
                ostringstream os;
                os << "frame size" << SP << dec << sig_len << SP;
                os << "MISMATCH: expected frame size is" << SP << dec << chk_len;
                this->msg->report_inf(os.str());
                fcs_ok = false;
            }
            ...
            if (sig_obs != sig_exp)
            {
                ostringstream os;
                os << "frame size" << SP << dec << sig_len << SP;
                os << "MISMATCH: observed FCS is" << SP << hex << setfill('0') << uppercase << setw(8) << sig_obs;
                os << "; expected is" << SP << hex << setfill('0') << uppercase << setw(8) << sig_exp;
                this->msg->report_inf(os.str());
                fcs_ok = false;
            }
            ...
            if (sig_res != FCS_RESIDUE)
            {
                ostringstream os;
                os << "frame size" << SP << dec << sig_len << SP;
                os << "MISMATCH: observed residue is" << SP << hex << setfill('0') << uppercase << setw(8) << sig_res;
                os << "; expected is" << SP << hex << setfill('0') << uppercase << setw(8) << FCS_RESIDUE;
                this->msg->report_inf(os.str());
                fcs_ok = false;
            }
            ...
            {
                ostringstream os;
                os << "frame size" << SP << dec << sig_len << SP << "OK";
                this->msg->report_inf(os.str());
            }

            this->pass = this->pass & fcs_ok;

            if (chk_len >= this->max_size)
            {
                this->done = true;
                this->msg->report_inf("reached frame maximum size" + SP + to_string(this->max_size));
            }
